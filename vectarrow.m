function vectarrow(p0, p1, w, clr)
...
%   $Revision: 1.0
  ...
  if max(size(p0))==3
      if max(size(p1))==3
          ...
          y1 = p1(2);
          z1 = p1(3);
          plot3([x0;x1],[y0;y1],[z0;z1], clr);   % Draw a line between p0 and p1
          ...
          hold on
          plot3(hu(:),hv(:),hw(:), clr)  % Plot arrow head
          grid on
          xlabel('x')
          ylabel('y')
          zlabel('z')
  ...
  elseif max(size(p0))==2
      if max(size(p1))==2
          ...
          x1 = p1(1);
          y1 = p1(2);
          plot([x0;x1],[y0;y1], clr);   % Draw a line between p0 and p1
          ...
          hold on
          plot(hu(:),hv(:), clr)  % Plot arrow head
          grid on
          xlabel('x')
          ylabel('y')
          hold off
